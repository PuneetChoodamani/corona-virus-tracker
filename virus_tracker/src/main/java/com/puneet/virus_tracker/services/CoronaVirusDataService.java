package com.puneet.virus_tracker.services;
...
public class CoronaVirusDataService {
	...
	public void fetchData() throws IOException, InterruptedException {
		List<LocationStats> tempStats = new ArrayList<LocationStats>();
		...
		for (CSVRecord record : records) {
			...
			int currentDayCount = Integer.parseInt(record.get(record.size() - 1));
			int prevDayCount = Integer.parseInt(record.get(record.size() - 2));
			stats.setLatestCount(currentDayCount);
			stats.setDiffFromPrevDay(currentDayCount - prevDayCount);
			tempStats.add(stats);
		}
